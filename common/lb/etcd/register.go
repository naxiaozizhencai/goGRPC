...
import (
	"context"
	"encoding/json"
	"github/leel0330/grpcdemo/common"
...
func (r *EtcdRegister) Register(srvInfo model.ServerNodeInfo, ttl int64) (chan<- struct{}, error) {
	// check etcd addrs
...
func (r *EtcdRegister) GetServiceInfo() (model.ServerNodeInfo, error) {
	resp, err := r.cli.Get(context.Background(), util.BuildRegPath(r.srvInfo))
	if err != nil {
		return r.srvInfo, err
	}

	infoRes := model.ServerNodeInfo{}
...
func (r *EtcdRegister) register() error {
	...
	_, err = r.cli.Put(context.Background(), util.BuildRegPath(r.srvInfo), string(regData), clientv3.WithLease(r.leasesID))
...
func (r *EtcdRegister) unregister() error {
	_, err := r.cli.Delete(context.Background(), util.BuildRegPath(r.srvInfo))
