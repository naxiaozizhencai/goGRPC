...
func (r *EtcdResolver) start() (chan<- struct{}, error) {
	...
	r.keyPrifix = util.BuildPrefix(model.ServerNodeInfo{Name: r.SrvName, Version: r.SrvVersion})
...
func (r *EtcdResolver) sync() error {
	...
	for i := range getResp.Kvs {
		addr := resolver.Address{Addr: util.SplitPath(string(getResp.Kvs[i].Key), r.keyPrifix)}
...
func (r *EtcdResolver) update(evs []*clientv3.Event) {
	for _, ev := range evs {
		...
		addr := resolver.Address{Addr: util.SplitPath(key, r.keyPrifix)}
		switch ev.Type {
		case mvccpb.PUT:
			if !util.Exist(r.srvAddrsList, addr) {
				r.srvAddrsList = append(r.srvAddrsList, addr)
				if !r.usedForTest {
					r.NewAddress(r.srvAddrsList)
				}
		...
		case mvccpb.DELETE:
			if s, ok := util.Remove(r.srvAddrsList, addr); ok {
				r.srvAddrsList = s
				if !r.usedForTest {
					r.NewAddress(r.srvAddrsList)
				}
